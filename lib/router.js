...
module.exports.byConfig = (app, opts) => {
  ...
  Config(app, (pathRegexp, ctl) => {
    router.route(pathRegexp)
      .head(actionWrap(ctl.headAction))
      .options(actionWrap(ctl.optionsAction))
      .get(actionWrap(ctl.getAction))
      .post(actionWrap(ctl.postAction))
      .put(actionWrap(ctl.putAction))
      .delete(actionWrap(ctl.deleteAction));
  });
  return router;
};
